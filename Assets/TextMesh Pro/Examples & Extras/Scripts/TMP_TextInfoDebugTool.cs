...
// Since this script is used for visual debugging, we exclude most of it in builds.
        ...
        void DrawCharactersBounds()
        {
            ...
            for (int i = 0; i < textInfo.characterCount; i++)
            {
                ...
                // Get Bottom Left and Top Right position of the currentLevelData character
        ...
        void DrawWordBounds()
        {
            ...
            for (int i = 0; i < textInfo.wordCount; i++)
            {
                ...
                for (int j = 0; j < wInfo.characterCount; j++)
                {
                    ...
                    TMP_CharacterInfo currentLevelDataCharInfo = textInfo.characterInfo[characterIndex];
                    int currentLevelDataLine = currentLevelDataCharInfo.lineNumber;

                    bool isCharacterVisible = characterIndex > m_TextComponent.maxVisibleCharacters ||
                                              currentLevelDataCharInfo.lineNumber > m_TextComponent.maxVisibleLines ||
                                             (m_TextComponent.overflowMode == TextOverflowModes.Page && currentLevelDataCharInfo.pageNumber + 1 != m_TextComponent.pageToDisplay) ? false : true;

                    // Track Max Ascender and Min Descender
                    maxAscender = Mathf.Max(maxAscender, currentLevelDataCharInfo.ascender);
                    minDescender = Mathf.Min(minDescender, currentLevelDataCharInfo.descender);

                    if (isBeginRegion == false && isCharacterVisible)
                    {
                        isBeginRegion = true;

                        bottomLeft = new Vector3(currentLevelDataCharInfo.bottomLeft.x, currentLevelDataCharInfo.descender, 0);
                        topLeft = new Vector3(currentLevelDataCharInfo.bottomLeft.x, currentLevelDataCharInfo.ascender, 0);

                        //Debug.Log("Start Word Region at [" + currentLevelDataCharInfo.character + "]");
                        ...
                        if (wInfo.characterCount == 1)
                        {
                            ...
                            bottomRight = m_Transform.TransformPoint(new Vector3(currentLevelDataCharInfo.topRight.x, minDescender, 0));
                            topRight = m_Transform.TransformPoint(new Vector3(currentLevelDataCharInfo.topRight.x, maxAscender, 0));

                            // Draw Region
                            DrawRectangle(bottomLeft, topLeft, topRight, bottomRight, wordColor);

                            //Debug.Log("End Word Region at [" + currentLevelDataCharInfo.character + "]");
                        }
                    }
                    ...
                    else if (isBeginRegion && currentLevelDataLine != textInfo.characterInfo[characterIndex + 1].lineNumber)
                    {
                        isBeginRegion = false;
